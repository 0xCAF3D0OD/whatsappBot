...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  // node_modules/pinecone-router/dist/router.esm.js
  var I = class {
    params = {};
    path;
    handlers = [];
    constructor(o, f = {}) {
      this.path = o, Object.keys(f).forEach((u) => {
        this[u] = f[u];
      }), f.template && (this.programmaticTemplate = true);
    }
    template = "";
    templateTargetId = "";
    programmaticTemplate = false;
    handlersDone = false;
    cancelHandlers;
  };
  var M = I;
  ...
  function k(i, o) {
    let f = /(?:\?([^#]*))?(#.*)?$/, u = i.match(f), w = {}, _;
    if (u && u[1]) {
      let d = u[1].split("&");
      for (let x = 0; x < d.length; x++) {
        let l = d[x].split("=");
        w[decodeURIComponent(l[0])] = decodeURIComponent(l.slice(1).join("="));
      }
    }
    let P = L(i.replace(f, "")), m = L(o || ""), E = Math.max(P.length, m.length);
    for (let d = 0; d < E; d++) if (m[d] && m[d].charAt(0) === ":") {
      let x = m[d].replace(/(^:|[+*?]+$)/g, ""), l = (m[d].match(/[+*?]+$/) || {}).toString()[0], y = ~l.indexOf("+"), R = ~l.indexOf("*"), b = P[d] || "";
      if (!b && !R && (l.indexOf("?") < 0 || y)) {
        _ = false;
        break;
      }
      if (w[x] = decodeURIComponent(b), y || R) {
        w[x] = P.slice(d).map(decodeURIComponent).join("/");
        break;
      }
    } else if (m[d] !== P[d]) {
      _ = false;
    ...
    return _ === false ? false : w;
  }
  function T(i, ...o) {
    if (window.PineconeRouterMiddlewares) for (let f in window.PineconeRouterMiddlewares) {
      let u = window.PineconeRouterMiddlewares[f];
      if (u[i] == null) return;
      if (u[i](...o) == "stop") return "stop";
    }
  }
  function D(i) {
  ...
  function S(i) {
    let o = i.reactive({ version: "4.4.1", name: "pinecone-router", settings: { hash: false, basePath: "/", templateTargetId: null, interceptLinks: true }, notfound: new M("notfound"), routes: [], context: { route: "", path: "", params: {}, query: window.location.search.substring(1), hash: window.location.hash.substring(1), redirect(e) {
      return g(e), "stop";
    }, navigate(e) {
      g(e);
    } }, add(e, n) {
      if (this.routes.find((r) => r.path == e) != null) throw new Error("Pinecone Router: route already exist");
      return this.routes.push(new M(e, n)) - 1;
    }, remove(e) {
      this.routes = this.routes.filter((n) => n.path != e);
    }, loadStart: new Event("pinecone-start"), loadEnd: new Event("pinecone-end") });
    window.PineconeRouter = o;
    var f = {}, u = {};
    let w = /* @__PURE__ */ new Set();
    var _ = {};
    let P = (e, n, r = false) => (f[n] && !r ? f[n].then((t) => e.innerHTML = t) : f[n] = fetch(n).then((t) => t.ok ? t.text() : (D(t.statusText), null)).then((t) => t == null ? (u[n] = null, f[n] = null, null) : (u[n] = t, e.innerHTML = t, t)), f[n]), m = (e, n, r) => {
      if (w.has(n)) return;
      w.add(n);
      let t = e.content.cloneNode(true).firstElementChild;
    ...
    function d(e, n, r, t) {
      ...
      e.content.firstElementChild ? (m(e, n, t), l()) : r && (u[r] ? (e.innerHTML = u[r], m(e, n, t), l()) : _[r] ? _[r].then(() => m(e, n, t)) : P(e, r).then(() => m(e, n, t)).finally(() => l()));
    }
    let x = () => {
      document.dispatchEvent(o.loadStart);
    }, l = () => {
      document.dispatchEvent(o.loadEnd);
    }, y = (e) => !o.settings.hash && o.settings.basePath != "/" ? o.settings.basePath + e : e, R = (e) => o.routes.findIndex((n) => n.path == e);
    i.directive("route", (e, { expression: n, modifiers: r }, { effect: t, cleanup: s }) => {
      let a = n;
      if (T("onBeforeRouteProcessed", e, a), a.indexOf("#") > -1) throw new Error("Pinecone Router: A route's path may not have a hash character.");
      let c = C(r, "target", null) ?? window.PineconeRouter.settings.templateTargetId, h = document.getElementById(c);
      if (c && !h) throw new Error("Pinecone Router: Can't find an element with the suplied target ID: " + c);
      let p = null;
      ...
      e._x_PineconeRouter_route = a, e.content.firstElementChild != null && i.nextTick(() => {
        t(() => {
          v.handlersDone && o.context.route == a ? d(e, n, null, h) : E(e);
        });
      }), s(() => {
    ...
    }), i.directive("handler", (e, { expression: n }, { evaluate: r, cleanup: t }) => {
      ...
      else throw new Error(`Pinecone Router: Invalid handler type: ${typeof a}.`);
      for (let p = 0; p < s.length; p++) s[p] = s[p].bind(i.$data(e));
      let c = e._x_PineconeRouter_route, h = c == "notfound" ? o.notfound : o.routes[R(c)];
      h.handlers = s, t(() => {
        h.handlers = [], h.handlersDone = true, h.cancelHandlers = false;
      });
    }), i.directive("template", (e, { modifiers: n, expression: r }, { Alpine: t, effect: s, cleanup: a }) => {
      ...
      let c = r, h = C(n, "target", null) ?? window.PineconeRouter.settings.templateTargetId, p = document.getElementById(h);
      if (h && !p) throw new Error("Pinecone Router: Can't find an element with the suplied target ID: " + h);
      n.includes("preload") && (_[c] = P(e, c).finally(() => {
        _[c] = null, l();
      }));
      let v = e._x_PineconeRouter_route, H = v == "notfound" ? o.notfound : o.routes[R(v)];
      ...
      T("init"), o.settings.hash == false ? g(location.pathname, false, true) : g(location.hash.substring(1), false, true);
    }), window.addEventListener("popstate", () => {
      o.settings.hash ? window.location.hash != "" && g(window.location.hash.substring(1), true) : g(window.location.pathname, true);
    }), b();
    function b() {
      function e(n) {
        if (!n || !n.getAttribute) return;
        let r = n.getAttribute("href"), t = n.getAttribute("target");
      ...
      window.document.body.addEventListener("click", function(n) {
        ...
        r.handlersDone || (r.cancelHandlers = true, l());
        let t = n.target;
        do
          if (t.localName === "a" && t.getAttribute("href")) {
            if (window.PineconeRouter.settings.interceptLinks == false && !t.hasAttribute("x-link") || t.hasAttribute("data-native") || t.hasAttribute("native")) return;
            let s = e(t);
    ...
    async function g(e, n = false, r = false) {
      ...
      let t = o.routes.find((a) => {
        let c = k(e, a.path);
        return a.params = c != false ? c : {}, c != false;
      }) ?? o.notfound;
      ...
      if (!n) {
        let a = "";
        if (o.settings.hash ? (a = "#", a += window.location.search + e) : a = e + window.location.search + window.location.hash, !r) history.pushState({ path: a }, "", a);
        else if (o.settings.hash && e == "/") return o.context = s, g("/", false, false);
      }
      if (t && t.handlers.length) {
        if (t.cancelHandlers = false, !await W(t.handlers, s)) {
          l();
          return;
        }
        t.handlersDone = true, t.template || l();
      }
      if (t.template && t.programmaticTemplate) {
        let a = t.templateTargetId ? document.getElementById(t.templateTargetId) : document.getElementById(o.settings.templateTargetId);
        u[t.template] ? (a.innerHTML = u[t.template], l()) : P(a, t.template).then(() => {
          l();
        });
      }
    ...
    function $(e, n, r) {
      return { route: e, path: n, params: r, query: window.location.search.substring(1), hash: window.location.hash.substring(1), redirect(t) {
        return g(t), "stop";
      }, navigate(t) {
        g(t);
      } };
    ...
    async function W(e, n) {
      for (let r = 0; r < e.length; r++) if (typeof e[r] == "function") {
        let t = o.routes[R(n.route)] ?? o.notfound;
        if (t.cancelHandlers) return t.cancelHandlers = false, false;
        let s;
        if (e[r].constructor.name === "AsyncFunction" ? s = await e[r](n) : s = e[r](n), s == "stop") return false;
      }
      return true;
    }
  }
  ...
  module_default.plugin(q);
  document.addEventListener("alpine:init", () => {
    module_default.data("Header", Header);
    module_default.data("WelcomeMessage", WelcomeMessage);
    module_default.data("DateTimeDisplay", DateTimeDisplay);
    module_default.data("CoverImage", CoverImage);
    module_default.data("ConnectionTest", ConnectionTest);
    module_default.data("FeaturesGrid", FeaturesGrid);
    module_default.data("Footer", Footer);
